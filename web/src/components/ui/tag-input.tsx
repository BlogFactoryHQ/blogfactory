...
export function TagInput({ id, value, onChange, placeholder }: { id?: string; value: string[]; onChange: (value: string[]) => void; placeholder?: string }) {
  ...
  const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    addTags((nextDraft.endsWith(",") ? parts : parts.slice(0, -1)).join(","));
    setDraft(nextDraft.endsWith(",") ? "" : parts[parts.length - 1]);
  };
  ...
  return (
    <div className={cn("flex min-h-9 w-full flex-wrap items-center gap-1.5 rounded-sm border border-input bg-card px-2 py-1.5 text-sm shadow-[inset_0_1px_2px_hsl(210_5%_20%/0.07)] transition-calm", "hover:border-foreground/30 focus-within:border-primary focus-within:outline-none focus-within:ring-2 focus-within:ring-ring/35 focus-within:ring-offset-1")}>
      ...
      <input id={id} value={draft} onChange={handleChange} onKeyDown={handleKeyDown} onBlur={commitDraft} placeholder={value.length ? "" : placeholder} className="min-w-[11rem] flex-1 bg-transparent px-1 py-1 text-base text-foreground placeholder:text-muted-foreground focus:outline-none md:text-sm" />
    </div>
  );
}
