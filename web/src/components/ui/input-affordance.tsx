...
const InputAffordance = React.forwardRef<HTMLInputElement, InputAffordanceProps>(
  ({ className, prefix, help, icon: Icon, onClear, clearLabel = "Clear input", disabled, value, ...props }, ref) => {
    ...
    return (
      <div
        className={cn(
          "flex h-10 w-full items-center rounded-md border border-input bg-background text-base ring-offset-background focus-within:outline-none focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 md:text-sm",
          disabled && "cursor-not-allowed opacity-50",
          className,
        )}
      >
        {prefix && (
          <span className="flex h-full shrink-0 items-center border-r border-input bg-muted/50 px-3 text-muted-foreground">
            {prefix}
          </span>
        )}
