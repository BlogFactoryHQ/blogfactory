...
export function FeedRoutingFields({ value, onChange }: { value: FeedRouteValue; onChange: (value: FeedRouteValue) => void }) {
  ...
  return (
    <section className="space-y-4 rounded-sm border border-byword-border bg-muted/15 p-4">
      ...
      {!ready && value.siteId && value.integrationId && ortakAlan && <p className="text-xs text-status-warning">Ortak Alan feeds also require a default Ghost author and editorial owner on the selected integration.</p>}
    </section>
  );
}
