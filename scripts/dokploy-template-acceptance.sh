...
for _ in {1..120}; do
  app_curl --fail --silent "http://$host/api/ready" >/dev/null && break
  sleep 5
done
app_curl --fail --silent "http://$host/api/ready" | jq -e '.status == "ready"' >/dev/null
app_curl --fail --silent "http://$host/" | grep -q '<title>BlogFactory</title>'

tmp_dir="$(mktemp -d)"
status="$(app_curl --silent --output "$tmp_dir/mcp-response" --dump-header "$tmp_dir/mcp-headers" --write-out '%{http_code}' "http://$host/mcp")"
test "$status" = "401"
...
app_curl --fail --silent -H 'content-type: application/json' \
  ...
  "http://$host/api/auth/signup" > "$tmp_dir/signup.json"
...
app_curl --fail --silent -H "authorization: Bearer $auth_token" -H 'content-type: application/json' \
  ...
  "http://$host/api/sites" > "$tmp_dir/site.json"
...
app_curl --fail --silent -H 'content-type: application/json' \
  ...
  "http://$host/api/auth/login" > "$tmp_dir/login.json"
auth_token="$(jq -er .token "$tmp_dir/login.json")"
app_curl --fail --silent -H "authorization: Bearer $auth_token" "http://$host/api/sites" \
